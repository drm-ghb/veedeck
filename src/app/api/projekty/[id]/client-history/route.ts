...
export async function GET(
  ...
  const clientEmails = project.clients
    .map((c) => c.email)
  ...
  const [
    ...
    // Pins + chat comments from clients (not designer, not internal, not AI)
    prisma.comment.findMany({
      ...
      select: {
        ...
        createdAt: true,
        renderId: true,
        render: { select: { name: true } },
      },
      orderBy: { createdAt: "desc" },
      take: 200,
    }),

    // Comments on shopping list products
    ...
    prisma.statusChangeRequest.findMany({
      where: { projectId },
      select: { id: true, renderName: true, clientName: true, createdAt: true, renderId: true, status: true },
      orderBy: { createdAt: "desc" },
      take: 100,
    }),
    ...
    // Login logs for known client emails
    ...
    // ClientEvents (render_view, list_view, product_approved, product_rejected)
  ...
  for (const c of comments) {
    ...
    events.push({
      ...
      detail: c.content,
      actor: c.author,
      link: `/projekty/${projectId}/renders/${c.renderId}`,
  ...
  for (const sr of statusRequests) {
    events.push({
      ...
      detail: null,
      actor: sr.clientName,
      link: `/projekty/${projectId}/renders/${sr.renderId}`,
  ...
  for (const vr of versionRequests) {
    events.push({
      ...
      detail: null,
      actor: vr.clientName,
      link: `/projekty/${projectId}/renders/${vr.renderId}`,
  ...
  for (const log of loginLogs) {
    const clientName = project.clients.find((c) => c.email === log.email)?.name ?? null;
    events.push({
      id: `login-${log.id}`,
      type: "login",
