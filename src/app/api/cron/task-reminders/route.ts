...
export async function GET(req: Request) {
  ...
  // Fetch all designer users with their reminder settings
  const designers = await prisma.user.findMany({
    where: { role: "designer" },
    select: { id: true, taskReminderHours: true },
  });

  if (designers.length === 0) return NextResponse.json({ ok: true, notificationsCreated: 0 });

  const reminderMap = new Map(
    designers.map((u) => [
      u.id,
      (u.taskReminderHours as Record<string, number> | null) ?? { LOW: 72, MEDIUM: 48, HIGH: 24 },
    ])
  );

  // Fetch all pending tasks with deadlines owned by designers
  const tasks = await prisma.task.findMany({
    where: {
      ownerId: { in: designers.map((u) => u.id) },
      dueDate: { not: null },
      status: { not: "DONE" },
      parentId: null,
    },
    select: { id: true, title: true, priority: true, dueDate: true, assigneeId: true, ownerId: true },
  });
  ...
  for (const task of tasks) {
    ...
    const reminderHours = reminderMap.get(task.ownerId) ?? { LOW: 72, MEDIUM: 48, HIGH: 24 };
