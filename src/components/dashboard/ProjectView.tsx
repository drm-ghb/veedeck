...
export default function ProjectView({ projectId, rooms, archivedRooms, allRenders }: ProjectViewProps) {
  ...
  async function handleRestore(roomId: string) {
    ...
    if (res.ok) {
      toast.success(t.projekty.roomRestored);
      router.refresh();
    } else {
      toast.error(t.projekty.roomRestoreError);
    }
  }
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {uniqueRooms.length > 1 && (
              ...
              >
                <option value="all">{t.projekty.allRooms}</option>
                {uniqueRooms.map((r) => (
          ...
          {filteredRenders.length === 0 ? (
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-sm mt-1">{t.projekty.noFilesSearchHint}</p>
            </div>
          ) : viewMode === "grid" ? (
